import React from "react";
import { FaCheck } from "react-icons/fa";

export default function ServiceFiverr() {
  return (
    <section className="bg-white dark:bg-gray-900 pt-10 fade-in">
      <div className="py-8 px-4 mx-auto max-w-screen-xl lg:py-16 lg:px-6">
        <div className="mx-auto max-w-screen-md text-center mb-8 lg:mb-12">
          <h2 className="mb-4 text-4xl tracking-tight font-extrabold text-gray-900 dark:text-white">
            Front End Development
          </h2>
          <p className="mb-5 font-light text-gray-500 sm:text-xl dark:text-gray-400">
            Elevate your web presence with cutting-edge design and seamless user experience.
            I will ensure your dream website is responsive, user-friendly and runs faster than a cheetah.
          </p>
        </div>
        <div className="space-y-8 lg:grid lg:grid-cols-3 sm:gap-6 xl:gap-10 lg:space-y-0">
          <div className="flex flex-col p-6 mx-auto max-w-lg text-center text-gray-900 bg-white rounded-lg border border-gray-100 shadow dark:border-gray-600 xl:p-8 dark:bg-gray-800 dark:text-white w-full">
            <h3 className="mb-4 text-2xl font-semibold">Starter</h3>
            <p className="font-light text-gray-500 sm:text-lg dark:text-gray-400">
              Best option for small sized projects
            </p>
            <div className="flex justify-center items-baseline my-8">
              <span className="mr-2 text-5xl font-extrabold">$80</span>
            </div>
            <ul role="list" className="mb-8 space-y-4 text-left">
              <li className="flex items-center space-x-3">
                <FaCheck className="flex-shrink-0 text-sm text-green-500 dark:text-green-400"/>
                <span>Content upload</span>
              </li>
              <li className="flex items-center space-x-3">
              <FaCheck className="flex-shrink-0 text-sm text-green-500 dark:text-green-400"/>
                <span>2 plugins/extensions</span>
              </li>
              <li className="flex items-center space-x-3">
                <FaCheck className="flex-shrink-0 text-sm text-green-500 dark:text-green-400"/>
                <span>
                  Delivery: <span className="font-semibold">2 days</span>
                </span>
              </li>
              <li className="flex items-center space-x-3">
                <FaCheck className="flex-shrink-0 text-sm text-green-500 dark:text-green-400"/>
                <span>
                  4 pages
                </span>
              </li>
              <li className="flex items-center space-x-3">
              <FaCheck className="flex-shrink-0 text-sm text-green-500 dark:text-green-400"/>
                <span>
                  Speed optimization
                </span>
              </li>
              <li className="flex items-center space-x-3">
                <FaCheck className="flex-shrink-0 text-sm text-green-500 dark:text-green-400"/>
                <span>
                  Unlimited Revisions
                </span>
              </li>
              <li className="flex items-center space-x-3">
                <FaCheck className="flex-shrink-0 text-sm text-green-500 dark:text-green-400"/>
                <span>
                  Social Media Integration
                </span>
              </li>
              <li className="flex items-center space-x-3">
                <FaCheck className="flex-shrink-0 text-sm text-green-500 dark:text-green-400"/>
                <span>
                  Opt-In Form
                </span>
              </li>
            </ul>
            <a
              href="https://www.fiverr.com/hamid_azim/do-front-end-development"
              target="_blank"
              className="text-white bg-green-600 hover:bg-primary-700 focus:ring-4 focus:ring-primary-200 font-medium rounded-lg text-sm px-5 py-2.5 text-center dark:text-white  dark:focus:ring-primary-900"
            >
              Get started
            </a>
          </div>
          <div className="flex flex-col p-6 mx-auto max-w-lg text-center text-gray-900 bg-white rounded-lg border border-gray-100 shadow dark:border-gray-600 xl:p-8 dark:bg-gray-800 dark:text-white w-full">
            <h3 className="mb-4 text-2xl font-semibold">Starter</h3>
            <p className="font-light text-gray-500 sm:text-lg dark:text-gray-400">
              Best option for mid sized projects.
            </p>
            <div className="flex justify-center items-baseline my-8">
              <span className="mr-2 text-5xl font-extrabold">$85</span>
            </div>
            <ul role="list" className="mb-8 space-y-4 text-left">
              <li className="flex items-center space-x-3">
                <FaCheck className="flex-shrink-0 text-sm text-green-500 dark:text-green-400"/>
                <span>Content upload</span>
              </li>
              <li className="flex items-center space-x-3">
              <FaCheck className="flex-shrink-0 text-sm text-green-500 dark:text-green-400"/>
                <span>4 plugins/extensions</span>
              </li>
              <li className="flex items-center space-x-3">
                <FaCheck className="flex-shrink-0 text-sm text-green-500 dark:text-green-400"/>
                <span>
                  Delivery: <span className="font-semibold">7 days</span>
                </span>
              </li>
              <li className="flex items-center space-x-3">
                <FaCheck className="flex-shrink-0 text-sm text-green-500 dark:text-green-400"/>
                <span>
                  6 pages
                </span>
              </li>
              <li className="flex items-center space-x-3">
              <FaCheck className="flex-shrink-0 text-sm text-green-500 dark:text-green-400"/>
                <span>
                  Speed optimization
                </span>
              </li>
              <li className="flex items-center space-x-3">
                <FaCheck className="flex-shrink-0 text-sm text-green-500 dark:text-green-400"/>
                <span>
                  Unlimited Revisions
                </span>
              </li>
              <li className="flex items-center space-x-3">
                <FaCheck className="flex-shrink-0 text-sm text-green-500 dark:text-green-400"/>
                <span>
                  Social Media Integration
                </span>
              </li>
              <li className="flex items-center space-x-3">
                <FaCheck className="flex-shrink-0 text-sm text-green-500 dark:text-green-400"/>
                <span>
                  Opt-In Form
                </span>
              </li>
            </ul>
            <a
              href="https://www.fiverr.com/hamid_azim/do-front-end-development"
              target="_blank"
              className="text-white bg-green-600 hover:bg-primary-700 focus:ring-4 focus:ring-primary-200 font-medium rounded-lg text-sm px-5 py-2.5 text-center dark:text-white  dark:focus:ring-primary-900"
            >
              Get started
            </a>
          </div>{/* <!-- Pricing Card --> */}
          <div className="flex flex-col p-6 mx-auto max-w-lg text-center text-gray-900 bg-white rounded-lg border border-gray-100 shadow dark:border-gray-600 xl:p-8 dark:bg-gray-800 dark:text-white w-full">
            <h3 className="mb-4 text-2xl font-semibold">Starter</h3>
            <p className="font-light text-gray-500 sm:text-lg dark:text-gray-400">
              Best option for larger projects.
            </p>
            <div className="flex justify-center items-baseline my-8">
              <span className="mr-2 text-5xl font-extrabold">$90</span>
            </div>
            <ul role="list" className="mb-8 space-y-4 text-left">
              <li className="flex items-center space-x-3">
                <FaCheck className="flex-shrink-0 text-sm text-green-500 dark:text-green-400"/>
                <span>Content upload</span>
              </li>
              <li className="flex items-center space-x-3">
              <FaCheck className="flex-shrink-0 text-sm text-green-500 dark:text-green-400"/>
                <span>6 plugins/extensions</span>
              </li>
              <li className="flex items-center space-x-3">
                <FaCheck className="flex-shrink-0 text-sm text-green-500 dark:text-green-400"/>
                <span>
                  Delivery: <span className="font-semibold">10 days</span>
                </span>
              </li>
              <li className="flex items-center space-x-3">
                <FaCheck className="flex-shrink-0 text-sm text-green-500 dark:text-green-400"/>
                <span>
                  8 pages
                </span>
              </li>
              <li className="flex items-center space-x-3">
              <FaCheck className="flex-shrink-0 text-sm text-green-500 dark:text-green-400"/>
                <span>
                  Speed optimization
                </span>
              </li>
              <li className="flex items-center space-x-3">
                <FaCheck className="flex-shrink-0 text-sm text-green-500 dark:text-green-400"/>
                <span>
                  Unlimited Revisions
                </span>
              </li>
              <li className="flex items-center space-x-3">
                <FaCheck className="flex-shrink-0 text-sm text-green-500 dark:text-green-400"/>
                <span>
                  Social Media Integration
                </span>
              </li>
              <li className="flex items-center space-x-3">
                <FaCheck className="flex-shrink-0 text-sm text-green-500 dark:text-green-400"/>
                <span>
                  Opt-In Form
                </span>
              </li>
            </ul>
            <a
              href="https://www.fiverr.com/hamid_azim/do-front-end-development"
              target="_blank"
              className="text-white bg-green-600 hover:bg-primary-700 focus:ring-4 focus:ring-primary-200 font-medium rounded-lg text-sm px-5 py-2.5 text-center dark:text-white  dark:focus:ring-primary-900"
            >
              Get started
            </a>
          </div>        </div>
      </div>
    </section>
  );
}
